...
export const CONTRACT_ABI = [
  ...
  parseAbiItem('event ExpenseAttested(bytes32 indexed submissionHash, string cid, address indexed submitter, uint256 timestamp)'),
] as const;

export const EXPENSE_ATTESTED_EVENT = parseAbiItem(
  'event ExpenseAttested(bytes32 indexed submissionHash, string cid, address indexed submitter, uint256 timestamp)'
);
